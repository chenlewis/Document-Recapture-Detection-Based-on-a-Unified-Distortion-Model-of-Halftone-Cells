...
halftoneAngle = 45;
imgBlkSize = 500;          % the block size of the image
...
scanned_halftone_dir_name = 'D:\Databases\HalftoneImageDatabase\scanned\ps_ht_doc_45_73_600_1200_crop_500\';
dataFileName = 'data_1st_doc_45_73_500_1200_space_7.mat';
...
% for non-empty data_collection: then add the names of the new filenames in the data collection cell array
  ...
  for scanned_halftone_dir_ind = 1:length(scanned_halftone_dir)
    ...
    disp(scanned_halftone_dir_ind / length(scanned_halftone_dir))
  end
end

%% remove certain entries from the datacollection

% idxToDelete = [];
% for ii = 1 : length(data_collection)
%   fileName = data_collection{ii};
%   % get the filename separater and extension
%   separatorIdx = strfind(fileName, '_');
%   extIdx = strfind(fileName, '.'); 
%   if strcmp(fileName(1:(extIdx(1) - 1)), 'hpm401n_epson850_5')
%     idxToDelete = [idxToDelete ii];
%   else
%     continue;
%   end
% end
% data_collection(idxToDelete, :) = [];
...
%   % first print
%   if strcmp(fileName((separatorIdx(1) + 1):(separatorIdx(2) - 1)), 'epson850') ...        % scanner
%       && strcmp(fileName(1:(separatorIdx(1) - 1)), 'hp6222') ...                        % printer
%       && strcmp(fileName((separatorIdx(2) + 1):(extIdx(1) - 1)), '4')                   % doc index
%     disp(fileName)
%   else
%     continue;
%   end
  ...
  if strcmp(fileName, 'hp258_epson850_6.tif_7.bmp')
    disp(fileName);
  else 
    continue;
  end
...
%     threshold = double(255) - (255 * exp(-0.5));
%     threshold = 150;
%     imgInputBinary = imgInput >= threshold;
%     imgInputBinaryDenoise = imgInputBinary;
...
%   set(gca, 'FontSize', 25, 'FontSmoothing', 'on');  
%   set(gcf, 'units', 'pixels', 'OuterPosition', [0 0 600 600], 'Position', [0 0 600 600], 'Color', 'w');
    title('image denoise 2');
  end
    %% plot the spectrum of the gray-scale images
%     % calculate and plot the spectrum
%     img_spectrum=log(abs(fftshift(fft2(double(imgInput))))+1);
%     figure;
%     imshow(img_spectrum./max(img_spectrum(:)),'Colormap',colormap(parula),'InitialMagnification','fit');
%     hold on;
%     dim_x=size(img_spectrum,1);             % row
%     dim_y=size(img_spectrum,2);             % column
%     set(gca,'XTick',[1:dim_y/8:dim_y,dim_y],...
%             'YTick',[1:dim_x/8:dim_x,dim_x]); 
%     set(gca,'XtickLabel',{'-\pi','-3\pi/4','-\pi/2','-\pi/4','0','\pi/4','\pi/2','3\pi/4','\pi'},...
%             'YtickLabel',{'-\pi','-3\pi/4','-\pi/2','-\pi/4','0','\pi/4','\pi/2','3\pi/4','\pi'},...
%             'XMinorTick','on','YMinorTick','on'); 
%     set(gca,'XAxisLocation','top','FontSize',23,...
%         'FontSmoothing','on','Position',[0.1700 0.1 0.75 0.75]);  
%     xlabel('\omega_x'); 
%     ylabel('\omega_y'); 
%     xlim([floor(3*dim_y/8) ceil(5*dim_y/8)+1])
%     ylim([floor(3*dim_x/8) ceil(5*dim_x/8)+1])
%     axis on;
...
%     % normalize the halftone dot center to be centered at 0
%     halftone_dot_center_cluster_shifted_col = ...
%       [halftone_dot_center_cluster_shifted_col halftone_dot_center_cluster_vert{i} - mean(halftone_dot_center_cluster_vert{i})];
  ...
  if parallel == 0
    ...
    title('Clustering Result at Vertical Direction');
  end
...
%   num_halftone_cells_row_rough = floor(imgBlkSize / cell_size);
  ...
  % show the kmean result
  if parallel == 0
    xlabel('$x_c$', 'Interpreter', 'latex');
    set(gca, 'FontName', 'Times', 'FontSize', 25, 'FontSmoothing', 'on');  
    set(gcf, 'units', 'pixels','OuterPosition', [0 0 1000 600], 'Position', [0 0 1000 600], 'Color', 'w');
    title('Clustering Result at Horizontal Direction');
  end
...
%                                  length(current_row_halftone_width)];
  ...
  if parallel == 0
    ...
    % plot the halftone cell detection result
    figure;
    imshow(imgInput);
    hold on;
    ...
    title(' rows where halftone cells are detected ');
    % plot the halftone cell detection result refined
    figure;
    imshow(imgInput);
    hold on;
    for row_ind = 1:length(rowCenterToCheck)
      plot(1:size(imgInput,2), ones(1, size(imgInput, 2)) .* rowCenterToCheck(row_ind), 'r', 'LineWidth', 2);
      hold on;
    end
    title('refined rows that are checked');
  end
...
%     % if using the fixed period
%     peak_f_idx(i) = peak_search_ind;
%     peak_value_log(i) = halftone_dot_spectrum_mean_log(peak_search_ind);

%     % try to find abnormal peaks
%     [pkAmp, pkLoc] = findpeaks(halftone_dot_spectrum_mean_log(local_search_range_ind), ...
%                                f(local_search_range_ind), 'Annotate', 'extents', 'MinPeakHeight', 3);
% 
%     % potential issue in the scanner: the largest and the second largest peaks differ by
%     % less than one. (this might not be importnat for now).
%     if ~isempty(pkAmp)
%       pkAmpRest = pkAmp;
%       pkAmpRest(pkAmpRest == max(pkAmp)) = [];
%       if any((max(pkAmp) - pkAmpRest) < 1)
%         figure;
%         findpeaks(halftone_dot_spectrum_mean_log(local_search_range_ind), ...
%                           f(local_search_range_ind), 'Annotate', 'extents', ...
%                           'MinPeakHeight', 3);
%         scannerIssue = 1;
%       end
%     end
...
%             cos_sum_error_analysis{i} = cos_sum_error_analysis{i} + cos(2*pi.*f.*T.*(N_sum_ind(n)-N_sum_ind(m)));
          end
        end
      end
  ...
  %   % the cos_sum is the fixed version
%   analytical_expression_refine_p1 = ...
%     (length(N_sum_ind)./(2.*(pi.*f).^2)).*(1-cos(2*pi.*f.*halftone_dot_width_mean_refined).*exp(-2.*(sigma_b_refined.*f.*pi).^2));
%   analytical_expression_refine_p2 = ...
%     sinc_modified(halftone_dot_width_mean_refined,f).^2.*exp(-(sigma_b_refined.*f.*pi).^2).*exp(-4.*(sigma_a_refined.*f.*pi).^2).*cos_sum;
%   analytical_expression_refine = analytical_expression_refine_p1 + analytical_expression_refine_p2;
%   analytical_expression_refine_log = log(analytical_expression_refine + 1);
  ...
  if parallel == 0
    ...
    set(gca, 'FontName', 'times', 'XLimMode', 'manual', ...
             'XLim', [-8 * (1 / T) 8 * (1 / T)], 'YLim', [1 max(analytical_value_refine.^log(10)) + 1000], 'FontSize', 26, 'FontSmoothing', 'on');  
    set(gcf, 'units', 'pixels','OuterPosition', [0 0 1300 900], 'Position', [0 0 1300 900], 'Color', 'w');
    xlabel('normalized frequency');
    ylabel('$\tilde{E}(f)$', 'Interpreter', 'latex');  
  end

%   %% detect the peaks in the PSD
%   figure;
%   findpeaks(halftone_dot_spectrum_mean_log, f, 'Annotate', 'extents');
%   xlim([-8 * (1 / T) 8 * (1 / T)]);
%   set(gca,'FontSize', 23, 'FontSmoothing', 'on');

  %% collect data # parallel #
  disp(['min loss is: ', num2str(minLoss)])
  data_collection_temp{2} = scannerIssue;
  data_collection_temp{3} = kmeanIssue;
...
% show_progress(data_collection, num_img_blks)
%% save data
save(dataFileName, 'data_collection');
